...
# GEO Site Architecture Audit 中文说明

> 从客户现有网站结构出发，判断哪些页面和内链需要补，才能更适合 AI 搜索理解和引用。

## 它解决什么问题

- 很多成熟网站并不缺内容，但页面结构、导航、帮助中心、产品页之间没有很好地回答 AI 用户的决策问题。
- 这个项目不会直接说“多发文章”，而是先看现有站点结构，再判断哪里缺页面、缺证据、缺内链。

## 什么时候用它

- 客户网站已有不少页面，不适合给一套泛泛的内容建议。
- 你想知道哪些页面可以变成 AI-citable evidence assets。
- 你想输出适合老板看的 P0 架构建议。

## 和相邻项目有什么区别

- 它从“现有网站结构”开始。
- `dageno-online-topic-prompt-generator` 从网站生成监控 Topic/Prompt。
- `seo-geo-content-engine` 更偏具体内容生产。

## 主要输出

- business profile
- site architecture map
- missing decision questions
- GEO site optimization plan
- P0 page/internal-link recommendations

## 快速开始

1. 准备客户官网、导航、sitemap、核心 landing pages、帮助中心和博客入口。
2. 先生成 business profile 和 architecture map。
3. 再输出 GEO-ready 页面和内链建议。
